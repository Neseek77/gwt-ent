...
public class ConstraintDescriptorImpl<T extends Annotation> implements ConstraintDescriptor<T>, Serializable {
	...
	public ConstraintDescriptorImpl(T annotation, ConstraintHelper constraintHelper, Class<?> implicitGroup, ElementType type, ConstraintOrigin definedOn) {
		...
		this.isReportAsSingleInvalidConstraint = ReflectionUtils.getAnnotation(annotation.annotationType(), ReportAsSingleViolation.class) != null; //annotation.annotationType().isAnnotationPresent(ReportAsSingleViolation.class	);
...
//			}
	...
	private List<Class<? extends ConstraintValidator<T, ?>>> findConstraintValidatorClasses() {
		...
		if ( constraintHelper.isBuiltinConstraint( annotation.annotationType() ) ) {
			...
			Class<? extends ConstraintValidator<?, ?>>[] validatedBy = ReflectionUtils.getAnnotation(annotationType, Constraint.class).validatedBy(); //annotationType.getAnnotation( Constraint.class ).validatedBy();
	...
	private void ensureAttributeIsOverridable(Method m, OverridesAttribute overridesAttribute) {
		...
		Type returnTypeOfOverriddenConstraint = method.getReturnType();
		if ( !returnTypeOfOverriddenConstraint.equals( m.getReturnType() ) ) {
			String message = "The overriding type of a composite constraint must be identical to the overridden one. Expected " + returnTypeOfOverriddenConstraint + " found " + m.getReturnType();
	...
	private Set<ConstraintDescriptor<?>> parseComposingConstraints() {
		...
		for ( Annotation declaredAnnotation : type.getDeclaredAnnotations() ) {//annotation.annotationType().getDeclaredAnnotations() ) {
