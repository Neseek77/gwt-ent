...
/**
 * Keeps track of builtin constraints and their validator implementations, as
 * well as already resolved validator definitions.
 * 
...
public class ConstraintHelper {
	...
	private final HashMap<Class<? extends Annotation>, List<Class<? extends ConstraintValidator<?, ?>>>> builtinConstraints = new HashMap<Class<? extends Annotation>, List<Class<? extends ConstraintValidator<?, ?>>>>();

	private final HashMap<Class<? extends Annotation>, List<Class<? extends ConstraintValidator<? extends Annotation, ?>>>> constraintValidatorDefinitons = new HashMap<Class<? extends Annotation>, List<Class<? extends ConstraintValidator<? extends Annotation, ?>>>>();
	...
	public List<Class<? extends ConstraintValidator<? extends Annotation, ?>>> getBuiltInConstraints(Class<? extends Annotation> annotationClass) {
		...
		List<Class<? extends ConstraintValidator<? extends Annotation, ?>>> constraints = new ArrayList<Class<? extends ConstraintValidator<? extends Annotation, ?>>>(
				builtInList.size());
	...
	public boolean isMultiValueConstraint(Annotation annotation) {
		...
		final GetMethod getMethod = GetMethod.action(annotation.annotationType(), "value");
		final Method method;
		...
		if (method != null) {
			...
			try {
				...
				if (returnType != null && returnType.isArray() != null && returnType.isArray().getComponentType() != null && returnType.isArray().getComponentType().isAnnotation() != null) {
	...
	/**
	 * Checks whether a given annotation is a multi value constraint and returns
	 * the contained constraints if so.
	 * 
	 * @param annotation
	 *          the annotation to check.
	 * 
	 * @return A list of constraint annotations or the empty list if
	 *         <code>annotation</code> is not a multi constraint annotation.
	 */
	public <A extends Annotation> List<Annotation> getMultiValueConstraints(A annotation) {
		...
		if (method != null) {
			try {
				...
				if (returnType.isArray() != null && returnType.isArray().getComponentType() != null && returnType.isArray().getComponentType().isAnnotation() != null) {
					Annotation[] annotations = (Annotation[]) method.invoke(annotation);
					for (Annotation a : annotations) {
						if (isConstraintAnnotation(a) || isBuiltinConstraint(a.annotationType())) {
	 ...
	 * constraint annotations has to fulfill the following conditions:
	 * <ul>
	...
	public boolean isConstraintAnnotation(Annotation annotation) {
		...
		Constraint constraint = ClassHelper.AsClass(annotation.annotationType()).getAnnotation(Constraint.class);
		if (constraint == null) {
			return false;
		}
	...
	private void assertPayloadParameterExists(Annotation annotation) {
		try {
			...
			if (defaultPayload.length != 0) {
				String msg = annotation.annotationType().getName() + " contains Constraint annotation, but the payload "
	...
	private void assertGroupsParameterExists(Annotation annotation) {
		try {
			// final GetMethod getMethod = GetMethod.action(
			// annotation.annotationType(), "groups" );
			...
			if (defaultGroups.length != 0) {
				String msg = annotation.annotationType().getName() + " contains Constraint annotation, but the groups "
						+ "paramter default value is not the empty array.";
				throw new ConstraintDefinitionException(msg);
			}
		} catch (ClassCastException e) {
	...
	public <T extends Annotation> List<Class<? extends ConstraintValidator<T, ?>>> getConstraintValidatorDefinition(Class<T> annotationClass) {
		...
		List<Class<? extends ConstraintValidator<T, ?>>> constraintsValidators = new ArrayList<Class<? extends ConstraintValidator<T, ?>>>(list.size());
