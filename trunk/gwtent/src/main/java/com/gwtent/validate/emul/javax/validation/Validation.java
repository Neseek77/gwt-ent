...
public class Validation {
	...
	private static class ProviderSpecificBootstrapImpl<T extends Configuration<T>, U extends ValidationProvider<T>>
		...
		public T configure() {
			...
			for ( ValidationProvider provider : resolvers ) {
				//James - 
				//gwtent/trunk/gwtent/target/classes/com/gwtent/validate/emul/javax/validation/Validation.java'
	      //[ERROR] Line 210: The method isAssignableFrom(Class<capture#1-of ? extends ValidationProvider>) is undefined for the type Class<U>
	      //[ERROR] Line 211: The method cast(ValidationProvider) is undefined for the type Class<U>
				...
				//Changed from above line
					return ((ValidationProvider<T>)provider).createSpecializedConfiguration( state );
				//}
			}
	...
	private static class DefaultValidationProviderResolver implements ValidationProviderResolver {
		...
		public List<ValidationProvider<?>> getValidationProviders() {
			//James - If you go to here, please discuss with me, we can figure out a common way for discovery GWT JSR303 implement
			List<ValidationProvider<?>> providers = new ArrayList<ValidationProvider<?>>();
			providers.add(new GWTEntValidatorProvider());
...
//		public List<ValidationProvider<?>> getValidationProviders() {
//			
//			//James - this is no class loader in GWT, we comment it first
//			
