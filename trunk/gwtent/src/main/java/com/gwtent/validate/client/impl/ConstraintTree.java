...
import com.gwtent.validate.client.metadata.ConstraintDescriptorImpl;
import com.gwtent.validate.client.util.TypeUtils;
import com.gwtent.validate.client.util.ValidatorTypeHelper;

/**
 * Due to constraint composition a single constraint annotation can lead to a
 * whole constraint tree being validated. This class encapsulates such a tree.
 * 
...
public class ConstraintTree<A extends Annotation> {
	...
	private ConstraintTree(ConstraintDescriptorImpl<A> descriptor,
			ConstraintTree<?> parent) {
		this.parent = parent;
		this.descriptor = descriptor;
		this.constraintValidatorCache = new HashMap<ValidatorCacheKey, ConstraintValidator<A, ?>>();

		final Set<ConstraintDescriptorImpl<?>> composingConstraints = new HashSet<ConstraintDescriptorImpl<?>>();
		for (ConstraintDescriptor<?> composingConstraint : descriptor
				.getComposingConstraints()) {
			composingConstraints
					.add((ConstraintDescriptorImpl<?>) composingConstraint);
		}
	...
	private <U extends Annotation> ConstraintTree<U> createConstraintTree(
	...
	public <T, U, V> void validateConstraints(Type type,
		...
		for (ConstraintTree<?> tree : getChildren()) {
			...
			tree.validateConstraints(type, executionContext, localExecutionContext,
					tmpViolations);
		...
		// we could have a composing constraint which does not need its own
		// validator.
		if (!descriptor.getConstraintValidatorClasses().isEmpty()) {
			...
			// );
			// }
			ConstraintValidator<A, V> validator = getInitializedValidator(type,
					executionContext.getConstraintValidatorFactory());

			validateSingleConstraint(executionContext, localExecutionContext,
					constraintViolations, constraintValidatorContext, validator);
		}

		if (reportAsSingleViolation() && constraintViolations.size() > 0) {
			constraintViolations.clear();
			final String message = (String) getDescriptor().getAttributes().get(
					"message");
			MessageAndPath messageAndPath = new MessageAndPath(message,
					localExecutionContext.getPropertyPath());
			ConstraintViolation<T> violation = executionContext
					.createConstraintViolation(localExecutionContext, messageAndPath,
							descriptor);
			constraintViolations.add(violation);
		}
	}

	private <T, U, V> void validateSingleConstraint(
			GlobalExecutionContext<T> executionContext,
			LocalExecutionContext<U, V> localExecutionContext,
			List<ConstraintViolation<T>> constraintViolations,
		...
		boolean isValid;
		try {
			isValid = validator.isValid(localExecutionContext
					.getCurrentValidatedValue(), constraintValidatorContext);
		} catch (RuntimeException e) {
			throw new ValidationException("Unexpected exception during isValid call",
					e);
		}
		if (!isValid) {
			constraintViolations.addAll(executionContext.createConstraintViolations(
	...
	/**
	 * @param type
	 *          The type of the value to be validated (the type of the
	 *          member/class the constraint was placed on).
	 * @param constraintFactory
	 *          constraint factory used to instantiate the constraint validator.
	 * 
	 * @return A initialized constraint validator matching the type of the value
	 *         to be validated.
	 */
	...
	private <V> ConstraintValidator<A, V> getInitializedValidator(Type type,
		...
		// check if we have the default validator factory. If not we don't use
		// caching (see HV-242)
		...
		ValidatorCacheKey key = new ValidatorCacheKey(constraintFactory,
				validatorClass);
		if (!constraintValidatorCache.containsKey(key)) {
			constraintValidator = createAndInitializeValidator(constraintFactory,
					validatorClass);
			...
			// }
			constraintValidator = (ConstraintValidator<A, V>) constraintValidatorCache
					.get(key);
		}
	...
	@SuppressWarnings("unchecked")
	private <V> ConstraintValidator<A, V> createAndInitializeValidator(
			ConstraintValidatorFactory constraintFactory,
			Class<? extends ConstraintValidator<?, ?>> validatorClass) {
		ConstraintValidator<A, V> constraintValidator;
		constraintValidator = (ConstraintValidator<A, V>) constraintFactory
	 ...
	 */
	private Class<? extends ConstraintValidator<?, ?>> findMatchingValidatorClass(
			Type type) {
		Map<Type, Class<? extends ConstraintValidator<?, ?>>> validatorTypes = ValidatorTypeHelper
	...
	private void verifyResolveWasUnique(Type valueClass,
			List<Type> assignableClasses) {
		if (assignableClasses.size() == 0) {
			String className = valueClass.toString();
			if (valueClass.isClass() != null) {
				Class<?> clazz = valueClass.isClass().getDeclaringClass();
//				if (clazz.isArray()) {
...
//				}
			...
			throw new UnexpectedTypeException(
					"No validator could be found for type: " + className);
		...
		// StringBuilder builder = new StringBuilder();
		// builder.append(
		// "There are multiple validator classes which could validate the type " );
		// builder.append( valueClass );
		// builder.append( ". The validator classes are: " );
		// for ( Type clazz : assignableClasses ) {
		// builder.append( clazz );
	...
	private void findSuitableValidatorTypes(Type type,
	 ...
	 *          The set of all classes which are assignable to the class of the
	 *          value to be validated and which are handled by at least one of the
	 *          validators for the specified constraint.
	 */
	...
	private <V> void initializeConstraint(ConstraintDescriptor<A> descriptor,
			ConstraintValidator<A, V> constraintValidator) {
		try {
	...
	private static class ValidatorCacheKey {
		...
		public boolean equals(Object o) {
			...
			if (constraintValidatorFactory != null ? !constraintValidatorFactory
					.equals(that.constraintValidatorFactory)
					: that.constraintValidatorFactory != null) {
				return false;
			}
		...
		@Override
		public int hashCode() {
			int result = constraintValidatorFactory != null ? constraintValidatorFactory
					.hashCode()
					: 0;
			result = 31 * result
					+ (validatorType != null ? validatorType.hashCode() : 0);
			return result;
		}
	}
}
