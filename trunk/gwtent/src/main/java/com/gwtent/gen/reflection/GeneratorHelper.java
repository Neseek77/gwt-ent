...
public class GeneratorHelper {
	 ...
	 * 
	 * <p>
	 * find a Parameterized class/interface in super classs/interfaces, this
	 * class should a sub class of ClassType class and will point out what's the
	 * class need generate reflection information
	 * 
	 * <p>
	 * if can NOT found, give a error, user should correct this before final
	 * compile
	 * 
	 * @param classType
	 * @return
	 */
	public static JClassType getReflectionClassType(TypeOracle oracle,
		...
		try {
			...
			throw new RuntimeException(
					"Can not found reflection class, forgot include module xml file?"
							+ e.getMessage());
		}

		ReflectionTarget target = classType
				.getAnnotation(ReflectionTarget.class);
		if (target != null) {
		...
		for (JClassType supClass : classType.getFlattenedSupertypeHierarchy()) {
			if (supClass.isParameterized() != null
					&& supClass.isAssignableTo(classClassType)) {
				if (supClass.isParameterized().getTypeArgs().length == 1) {
					...
					throw new RuntimeException(
							"ClassType should have only one Parameterized type, please see document of ClassType interface. Current processing type: "
									...
									+ ", Current parameterized type count:"
	...
	/**
	 * Give a array of JClassTypes, return the array of qualified source name of
	 * it.
	 * 
	 * @param types
	 * @return
	 ...
	 * 
	 * @param dest
	 *            field or method or class
	 * @param source
	 *            source to print code
	 * @param metaData
	 */
	...
	public static void addAnnotations_AnnotationImpl(
		...
		for (Annotation annotation : annotations) {
			JClassType classType = typeOracle.findType(ReflectionUtils
					.getQualifiedSourceName(annotation.annotationType()));
			if (classType != null) {
				source
						.print(dest
								+ ".addAnnotation("
								+ createAnnotationValues(typeOracle,
										annotation, logger) + ");");
				...
				logger
						.log(
								...
								"Annotation ("
										+ ReflectionUtils
												.getQualifiedSourceName(annotation
														.annotationType())
	...
	public static String createAnnotationValues(
			com.google.gwt.core.ext.typeinfo.TypeOracle typeOracle,
		...
		JClassType classType = typeOracle.findType(ReflectionUtils
				.getQualifiedSourceName(annotation.annotationType()));
		if (classType != null) {
			StringBuffer classTypeCode = TypeHelper.getClassTypeCode(classType
			...
			sb
					.append(""
							+ classTypeCode
							+ ",new com.gwtent.reflection.client.impl.AnnotationValues(");
			sb.append("\"" + classType.getQualifiedSourceName()
					+ "\", new Object[]{");
	 ...
	 * 
	 * primitive need take care. For example float 2.0, it need changed to 2.0F,
	 * otherwise it's becomes a Double
	 * 
	 * @param object
	 * @return
	 */
	public static String annoValueToCode(
			com.google.gwt.core.ext.typeinfo.TypeOracle typeOracle,
			Object object, TreeLogger logger) {
		if (object == null)
			return "null";

		if (object instanceof String) {
			return "\""
					+ object.toString().replace("\\", "\\\\").replace("\"",
							"\\\"") + "\"";
		} else if (object instanceof Class) {
			return ((Class) object).getCanonicalName() + ".class"; // inner
																	// class
																	// will got
																	// problem
		} else if (object.getClass().isArray()) {
			...
			sb.append("new ").append(compType.getCanonicalName())
					.append("[] {");
			for (int i = 0; i < Array.getLength(object); i++) {
				if (i > 0)
					sb.append(", ");
			...
			return createAnnotationValues(typeOracle, (Annotation) object,
					logger);
	...
	public static Reflectable getReflecInfo(JClassType classType) {
		Reflectable reflectable = GenUtils
				.getClassTypeAnnotationWithMataAnnotation(classType,
						Reflectable.class);
		if (reflectable == null) {
			// default value
			reflectable = Reflect_Full.class.getAnnotation(Reflectable.class);
		}
		return reflectable;
	}
